...
```{r}
cars_data <- mtcars
efficient <- cars_data[cars_data$mpg > 23, ]
```
...
```{r}
subset(cars_data, qsec <= 16.9) 
nrow(subset(cars_data, qsec <= 16.9))

subset(cars_data, qsec < 16.9 | qsec = 16.9) 
nrow(subset(cars_data, qsec < 16.9 | qsec == 16.9))
...
```{r}
lightCars <- subset(cars_data, wt < 2)
...
# this adds the year column and alternates teh value between 1973 and 1974
```
...
more code.
```{r}
mtcars$wt # all the weight value
cond <- mtcars$wt < 3 # find all the weights less than 3 (bolean)
cond #(cond = condtion?) prints the bolean
mtcars$weight_class <- ifelse(cond, 'light', 'average') #created new column
#condition, if true, weight_class = light, if false weight_class = average
mtcars$weight_class #prints out weight_class and it's values
cond <- mtcars$wt > 3.5 #new condtion, wt > 3.5
mtcars$weight_class <- ifelse(cond, 'heavy', mtcars$weight_class)
#same as line 131, but names > 3.5 has heavey
mtcars$weight_class #prints out weight_class
```
